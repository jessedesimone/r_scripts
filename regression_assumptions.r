...
reg.model <- lm(<response variable> ~ <predictor> , data=df)
...
check=function(lm.object,tests=FALSE){ ### Checking model assumptions
  ...
  if(tests==TRUE){
    ...
    if(class(lm.object)[1]=="aov")ConstantVar=levene.test(lm.object$model[,1],lm.object$model[,2])
